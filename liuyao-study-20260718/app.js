...
const state = {
  ...
  castMode:"random", manualCoins:["字","背","字"], lessonCoins:["阴","阴","阳"]
};
const learningModules = [
  ["lecture0704-main","旺衰关系专题"],
  ["lecture0718-main","起卦与七宫五十六卦"],
...
function renderCourse0718Chain(){
  const steps=[
    ["01","正确起卦","三钱定一爻，共投六次"],
    ["02","排六爻","第一次在下，第六次在上"],
    ["03","识结构","内外卦、三才、贱位与尊位"],
    ["04","定卦宫","八宫归属决定卦的宫五行"],
    ["05","读大象","上下卦之象合看，得到取象角度"],
    ["06","回到断卦","不单凭卦名定吉凶，等待用神与旺衰"]
  ];
  document.querySelector("#course0718Chain").innerHTML=steps.map((x,i)=>`<article style="--delay:${i*70}ms"><b>${x[0]}</b><h3>${x[1]}</h3><p>${x[2]}</p></article>`).join("");
}

function lessonCoinReading(){
  const yin=state.lessonCoins.filter(x=>x==="阴").length;
  if(yin===3)return course0718.coins[3];
  if(yin===2)return course0718.coins[0];
  if(yin===1)return course0718.coins[1];
  return course0718.coins[2];
}

function renderLessonCoins(flippingIndex=-1){
  const root=document.querySelector("#lessonCoins");
  root.innerHTML=state.lessonCoins.map((face,i)=>`<button type="button" class="lesson-coin ${face==="阴"?"is-yin":"is-yang"} ${i===flippingIndex?"is-flipping":""}" data-lesson-coin="${i}" aria-label="第 ${i+1} 枚，当前${face}面"><span class="lesson-coin-disc"></span><b>${face}</b><small>${face==="阴"?"汉字面":"满文面"}</small></button>`).join("");
  root.querySelectorAll("[data-lesson-coin]").forEach(button=>button.addEventListener("click",()=>{
    const i=Number(button.dataset.lessonCoin);
    state.lessonCoins[i]=state.lessonCoins[i]==="阴"?"阳":"阴";
    renderLessonCoins(i);
  }));
  const reading=lessonCoinReading();
  document.querySelector("#lessonCoinResult").innerHTML=`<span>${reading.faces}</span><strong>${reading.result} · ${reading.line}</strong><b>${reading.mark}</b><p>${reading.note}</p>`;
}

function renderCoinRuleGrid(){
  document.querySelector("#coinRuleGrid").innerHTML=course0718.coins.map((x,i)=>`<article class="${x.moving?"is-moving":""}"><b>0${i+1}</b><span>${x.faces}</span><h3>${x.result}</h3><strong>${x.mark}</strong><p>${x.note}</p></article>`).join("");
}

function renderYaoPositions(active=5){
  const stack=document.querySelector("#yaoStack");
  stack.innerHTML=course0718.yao.map(x=>`<button type="button" data-yao="${x.n}" class="${x.n===active?"active":""}"><i></i><b>${x.n}</b><span>${x.name}</span><small>${x.triad} · ${x.zone}</small></button>`).join("");
  stack.querySelectorAll("[data-yao]").forEach(b=>b.addEventListener("click",()=>renderYaoPositions(Number(b.dataset.yao))));
  const x=course0718.yao.find(y=>y.n===active);
  document.querySelector("#yaoInspector").innerHTML=`<span>${x.zone} · ${x.triad}</span><h3>${x.name}</h3><strong>${x.cue}</strong><p>${x.n===1?"起卦时第一次投掷放在最下方。":x.n===5?"五爻是本讲重点位置，可取君位、中央、大环境或政策层。":x.n===2?"陈师板书与口述均称二爻为贱位，与五爻尊位对照。":"位置之象需随所问事项转换太极点。"}</p>`;
}

function renderPalaceOrbit(){
  const root=document.querySelector("#palaceOrbit");
  root.innerHTML=`<div class="palace-orbit-core"><small>六十四卦</small><strong>八宫</strong><span>宫五行用于后续装六亲</span></div>`+course0718.palaceOrder.map((key,i)=>{
    const p=course0718.palaces[key];
    return `<button type="button" style="--i:${i}" data-palace-jump="${key}" class="${p.status==='pending'?"pending":""}" ${p.status==='pending'?'disabled aria-disabled="true"':''}><b>${key}</b><span>${p.element}</span><small>${p.status==='verified'?'8 卦已核对':'待下节'}</small></button>`;
  }).join("");
  root.querySelectorAll("[data-palace-jump]").forEach(b=>b.addEventListener("click",()=>{
    const key=b.dataset.palaceJump;
    if(course0718.palaces[key].status==='pending')return;
    renderHexagramAtlas(key,0);
    document.querySelector(".hexagram-atlas-section").scrollIntoView({behavior:"smooth",block:"start"});
  }));
}

function renderHexagramAtlas(palaceKey="乾",activeIndex=0){
  const tabs=document.querySelector("#palaceTabs");
  tabs.innerHTML=course0718.palaceOrder.map(key=>{
    const p=course0718.palaces[key];
    return `<button type="button" data-palace-tab="${key}" class="${key===palaceKey?"active":""} ${p.status==='pending'?"pending":""}" ${p.status==='pending'?'disabled aria-disabled="true"':''}><b>${key}宫</b><span>${p.element}</span><small>${p.status==='verified'?p.time:'待下节'}</small></button>`;
  }).join("");
  tabs.querySelectorAll("[data-palace-tab]").forEach(b=>b.addEventListener("click",()=>{
    const key=b.dataset.palaceTab;
    if(course0718.palaces[key].status==='pending')return;
    renderHexagramAtlas(key,0);
  }));
  const palace=course0718.palaces[palaceKey];
  document.querySelector("#hexagramSequence").innerHTML=palace.hexagrams.map((h,i)=>`<button type="button" data-hexagram-index="${i}" class="${i===activeIndex?"active":""}" style="--seq:${i}"><b>${String(i+1).padStart(2,'0')}</b><strong>${h[1]}</strong><span>${h[0]}</span><small>${h[2]}</small></button>`).join("");
  document.querySelectorAll("[data-hexagram-index]").forEach(b=>b.addEventListener("click",()=>renderHexagramAtlas(palaceKey,Number(b.dataset.hexagramIndex))));
  const h=palace.hexagrams[activeIndex];
  document.querySelector("#hexagramInspector").innerHTML=`<div class="hexagram-glyph">${h[1]}</div><span>${palaceKey}宫 · ${palace.element} · 第 ${activeIndex+1} 卦</span><h3>${h[0]}</h3><div><b>核心象</b><strong>${h[2]}</strong></div><p>${h[3]}</p><small>录制定位：${palace.time}</small>`;
}

function renderContrastGrid(){
  const items=[
    ["天地否","地天泰","阴阳不交 · 闭塞","阴阳交通 · 顺通"],
    ["水火既济","火水未济","当下已成 · 防物极必反","当下未成 · 未来有希望"],
    ["山天大畜","风天小畜","长周期积蓄与停留","短暂收敛与蓄力"],
    ["山地剥","山泽损","从高处剥落下跌","内空、减少与亏损"],
    ["坎为水","艮为山","险阻、深陷、坑坎","山重、难越、止步"],
    ["火地晋","地风升","向前晋升，测病防反取","向上晋升，测病防棺木入土取象"]
  ];
  document.querySelector("#contrastGrid").innerHTML=items.map((x,i)=>`<article><b>0${i+1}</b><div><span>${x[0]}</span><strong>${x[2]}</strong></div><i>⇄</i><div><span>${x[1]}</span><strong>${x[3]}</strong></div></article>`).join("");
}

function renderCourse0718(){
  renderCourse0718Chain();
  renderCoinRuleGrid();
  renderLessonCoins();
  renderYaoPositions();
  renderPalaceOrbit();
...
renderPath();renderElementImages();renderWuxing();renderRelativeTransformer();renderMap();renderSeasons();renderWheel();renderTrigrams();renderBranchRelationLab();renderChangsheng();renderHiddenStems();renderLectureTables();renderSeasonNotes();renderCourse0718();renderCoins();renderCast();renderRelatives();renderTopics();renderFilters();renderBranchGrid();renderFlashcard();renderLearningTracking();initProgressDetail();updateProgress();
initImmersiveMotion();
