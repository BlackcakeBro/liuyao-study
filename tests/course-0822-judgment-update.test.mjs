...
test("08-22 course records the start of judgment study and source boundary",()=>{
  ...
  assert.equal(course.meta.transcriptCoverage,"00:07–15:41");
  assert.equal(course.meta.visualEvidenceStatus,"unavailable");
  assert.match(course.meta.evidence,/画面轨未取得/);
